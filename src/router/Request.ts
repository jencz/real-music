...
export default class Request {
	 ...
	 */
	getId = () => {
		let idString = this.req.url?.split("/")[2]

		if (idString != null) {
			if (idString.includes("?")) { // checking if there's a question mark after the id
				return Number(idString.replace("?", ""))
			}
		}
	...
	getId2 = () => {
		let idString = this.req.url?.split("/")[3]
	...
	// http://localhost:3000/playlist/1/song/2 => [ 1, 2 ]

	// http://localhost:3000/playlist/1/song/2 => [ 1, 2 ]
	getIds = () => {

		let idString = this.req.url?.split("/")
		let ids: number[] | null = null
		if (idString) {
			ids = [Number(idString[2]), Number(idString[4])]
		}
	...
	*/
	...
	findCookie = (name: string) => {
		return this.getCookies().find((cookie) => cookie.name === name);
	};
